...
### 2. Execute One-Shot Workflow (with V1.5 Subagent Delegation)
...
9. **Commit via Approval Workflow**
...
Subagent delegation prevents context ballooning during multi-task batch execution. Without delegation, a 5-task batch could consume 90%+ of context; with delegation, the orchestrator stays under 50%.
...
IF affected files are unknown OR task spans multiple modules:
  ...
  → Receive summary (1-2K tokens)
  → Use summary for planning
```

**Implementation (Step 4):**
```
IF change >50 lines OR >3 files:
  → Delegate to IMPLEMENTER subagent
  → Receive summary (2-3K tokens)
  → Verify COMPLETE status before commit
```
...
|--------|-------------------|-----------------|
| Exploration per task | 10-20K tokens | 1-2K tokens |
| Implementation per task | 15-30K tokens | 2-3K tokens |
| Context after 5 tasks | 90%+ | <50% |

### Handling Subagent Results

**Explorer Summary → Use for planning:**
- File paths and descriptions
- Patterns to follow
- Dependencies noted

**Implementer Summary → Verify before commit:**
- Status: COMPLETE / PARTIAL / FAILED
- Files changed
- Test results
- If FAILED: halt batch, report to user
