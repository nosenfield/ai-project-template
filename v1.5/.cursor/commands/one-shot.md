...
2. ASSESS: Exploration Needed? (V1.5)
   ├─ Are affected files known?
   │   NO → Delegate to EXPLORER subagent
   │         └─ Receive exploration summary (1-2K tokens)
   │   YES → Continue to planning
   └─ Use exploration summary (do NOT re-read explored files)

3. Plan Implementation (SILENTLY)
   ├─ Files to modify (from exploration or direct knowledge)
   ├─ Implementation steps
   ├─ Test strategy
   └─ Risk assessment

4. ASSESS: Implementation Delegation? (V1.5)
   ├─ Is change >50 lines OR >3 files?
   │   YES → Delegate to IMPLEMENTER subagent
   │         ├─ Provide: requirements, exploration summary, file paths
   │         └─ Receive implementation summary (2-3K tokens)
   │   NO → Execute directly (step 5)
   └─ Use implementation summary (do NOT re-read implemented files)

5. Test-First Implementation (if not delegated)
   ├─ Write tests first (RED)
   ├─ Implement to pass tests (GREEN)
   └─ Verify all tests pass

6. Verify Implementation
   ├─ Run tests (retry up to 3 times if failures)
...
Subagent delegation prevents context ballooning during multi-task phase execution. Each task can consume significant context for exploration and implementation; subagents isolate this context and return concise summaries.

### Delegation Decision Points

**Step 2 - Exploration:**
```
IF affected files are unknown OR task spans multiple modules:
  → Delegate to EXPLORER subagent
  → Request format:
    - Target: [feature/system related to task]
    - Depth: medium
    - Context: Task [task-id], autonomous execution
  → Receive: File paths, patterns, dependencies (1-2K tokens)
  → Action: Use summary for planning, do NOT re-read explored files
```

**Step 4 - Implementation:**
```
IF implementation >50 lines OR >3 files:
  → Delegate to IMPLEMENTER subagent
  → Request format:
    - Task: [task-id] - [description]
    - Requirements: [from task definition]
    - Exploration summary: [from step 2]
    - Files to modify: [from planning]
    - Constraints: TDD, stay in scope, do NOT commit
  → Receive: Status, files changed, test results (2-3K tokens)
  → Action:
    - If COMPLETE: proceed to commit workflow
    - If FAILED: halt and report to user
```
...
|-----------|--------|---------|
| Exploration per task | 1-2K tokens | vs 10-20K without delegation |
| Implementation per task | 2-3K tokens | vs 15-30K without delegation |
| Orchestrator after 5 tasks | <50% context | vs 90%+ without delegation |

### Delegation Does NOT Change

- Commit approval workflow (unchanged)
- AUTO_ACCEPT rules (unchanged)
- Pre-commit hook behavior (unchanged)
- Phase completion procedures (unchanged)
